...
class ElavonCpiPaymentProviderSpec extends Specification {
    ...
    def "authorize flags the transaction as ad-hoc credential-on-file establishing"() {
        ...
        when:
        provider.authorize(BigDecimal.valueOf(75), "device-1", "Incidentals hold", null, null)
    ...
    def "chargeStoredCredential sends a merchant-initiated SALE via the gateway with the stored token"() {
        ...
        when:
        def result = provider.chargeStoredCredential(BigDecimal.valueOf(150), null, "stored-token-1", "Incident: broken TV", null, null)
    ...
    def "chargeStoredCredential returns a failed AuthResult when CPI declines"() {
        ...
        when:
        def result = provider.chargeStoredCredential(BigDecimal.valueOf(150), null, "stored-token-1", "Incident", null, null)
